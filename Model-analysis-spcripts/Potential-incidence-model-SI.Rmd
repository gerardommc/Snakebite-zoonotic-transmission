...
bites.incidence <- readRDS("../Model-results/Snakebites-mass-action-JAGS-model.rds")
...
$$\frac{\Delta H_{bitten}}{\Delta t} = H \times \left( 1 - \exp(- \beta S) \right)$$
...
Parameter estaimated with the JAGS run.
...
The `R` function to predict the model is:
...
indices <-  param.incidence[, paste0("indices.", 1:7, ".")]/100
...
s.mins.1 <- foreach(i = 1:5, .combine = c) %do% {min(S.sum.unw[incid.data$land.cover.num == i])[1]}
s.max.1 <- foreach(i = 1:5, .combine = c) %do% {max(S.sum.unw[incid.data$land.cover.num == i])[1]}
...
rates <- foreach(i = 1:nrow(incid.data), .combine = rbind) %do% {
      contact.rate(
            ...
            beta0 = param.incidence[, paste0("beta0.", incid.data$land.cover[i], ".")] +log(100),
...
rates.resp <- foreach(i = 1:5) %dopar% {
      x <- foreach(j = 1:1000, .combine = rbind) %do% {
             contact.rate(H.log = rates.resp.data[[i]]$H.log[j], 
                   beta0 = param.incidence[, paste0("beta0.", i, ".")] + log(100),
                   beta1 = param.incidence[, paste0("beta1.", i, ".")])     
      }
      return(x)
}
...
library(reshape2)
beta0 <- data.frame(param.incidence[, paste0("beta0.", 1:5, ".")]) + log(100)
